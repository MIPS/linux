...
{
	unsigned int cpu;
	int prev_cluster = -1, prev_core = -1, id = -1;

	for_each_possible_cpu(cpu) {
		if ((cpu_cluster(&cpu_data[cpu]) != prev_cluster) ||
		    (cpu_data[cpu].core != prev_core))
			id++;

		per_cpu(cpu_core_id, cpu) = id;

		prev_cluster = cpu_cluster(&cpu_data[cpu]);
		prev_core = cpu_data[cpu].core;
	}
}
