...
static void __init cps_prepare_cpus(unsigned int max_cpus)
{
	...
	if (cca_unsuitable || cpu_has_dc_aliases) {
		for_each_present_cpu(c) {
			if (!cpu_cluster(&cpu_data[c]) && !cpu_data[c].core)
				continue;
	...
	bitmap_set(cluster_bootcfg->core_power, current_cpu_data.core, 1);
	core_bootcfg = &cluster_bootcfg->core_config[current_cpu_data.core];
...
static int cps_cpu_disable(void)
{
	...
	if (cpu_has_mipsmt) {
		...
		for_each_cpu((stolen), &cpu_stolen_mask)
			if ((cpu_cluster(&cpu_data[stolen]) ==
			     cpu_cluster(&cpu_data[cpu])) &&
			    (cpu_data[stolen].core == cpu_data[cpu].core))
				siblings++;
	...
	core_cfg = &cluster_cfg->core_config[current_cpu_data.core];
