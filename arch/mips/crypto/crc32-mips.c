...
do {										\
	...
	"move	$at, %1\n\t"							\
	"# crc32* $1, %1\n\t"							\
	".word  0x7c00000f | (2 << 16) | (1 << 21) | (%2 << 6) | (%3 << 8)\n\t"	\
	".set	pop\n\t"							\
	: "+r" (crc)								\
...
#define CRC32(crc, value, size) \
	_CRC32(crc, value, size, crc_type_std)

#define CRC32C(crc, value, size) \
	_CRC32(crc, value, size, crc_type_castagnoli)
...
static u32 crc32_mips_le_hw(u32 crc_, const u8 *p, unsigned int len)
{
	...
	if (length & sizeof(u16)) {
		...
		CRC32(crc, value, crc_op_halfword);
		p += sizeof(u16);
	}
...
static u32 crc32c_mips_le_hw(u32 crc_, const u8 *p, unsigned int len)
{
	...
	if (length & sizeof(u16)) {
		...
		CRC32C(crc, value, crc_op_halfword);
		p += sizeof(u16);
	}
