...
static int cpu_starting(unsigned int cpu)
{
	...
	clockevents_register_device(&cd->dev);
...
static int __init mips_gic_watchdog_timer_init(struct device_node *node)
{
	...
	for_each_possible_cpu(cpu) {
		...
		cd->dev.min_delta_ticks = 0x300;
		cd->dev.max_delta_ticks = 0xffffffff;

		cd->dev.rating = 400;

		clockevents_config(&cd->dev, gic_frequency);
	}
...
CLOCKSOURCE_OF_DECLARE(mips_gic_watchdog_timer,
