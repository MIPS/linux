...
/*
 * Galileo GT-64120 PCI Controller Driver
...
static int gt64120_probe(struct platform_device *pdev)
{
	...
	bridge = devm_pci_alloc_host_bridge(dev, sizeof(*gtpci));
	if (!bridge)
		return -ENODEV;

	gtpci = pci_host_bridge_priv(bridge);
	...
	bridge->sysdata = gtpci;
	bridge->busnr = 0;
	bridge->ops = &gt64120_pci_ops;
...
static const struct of_device_id gt64120_of_match[] = {
